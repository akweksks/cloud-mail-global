...
# =============================================================================
# cloud-mail-global — One-Click Deploy Script
#
# What it does:
...
#   8. Saves state to .cloud-mail-global-deploy.env so re-runs skip prompts
#
# Usage:
...
STATE_FILE="$REPO_ROOT/.cloud-mail-global-deploy.env"
WORKER_NAME="cloud-mail-global"
...
preflight() {
  ...
  if command -v pnpm >/dev/null 2>&1; then
    ...
    PKG_MGR="npm"
    warn "pnpm not found, falling back to npm (cloud-mail-global uses pnpm-lock.yaml — install times will be slower)"
  else
...
save_state() {
  umask 077
  cat >"$STATE_FILE" <<EOF
# cloud-mail-global deploy state — do not commit (in .gitignore)
DOMAINS="${DOMAINS:-}"
ADMIN="${ADMIN:-}"
...
prompt_inputs() {
  ...
  D1_NAME="${D1_NAME:-cloud-mail-global}"
  KV_NAME="${KV_NAME:-cloud-mail-global-kv}"
  R2_BUCKET="${R2_BUCKET:-cloud-mail-global-r2}"
...
deploy_worker() {
  ...
  if [ -z "$WORKER_URL" ]; then
    ...
    read -rp "  Enter Worker URL (e.g. https://cloud-mail-global.<your>.workers.dev): " WORKER_URL
  fi
...
init_db() {
  ...
  local code
  # cloud-mail-global's init endpoint is GET /api/init/:secret
...
============================================
  cloud-mail-global — Deployment Complete!
============================================
...
$([ "${USE_AI:-false}" = "true" ] && echo "    AI:    [ai] binding bound to Workers AI"
  ...
  Next steps:
    ...
    2. Cloudflare Dashboard → Email → Email Routing
       Add catch-all rule for each domain → cloud-mail-global Worker
...
destroy_all() {
  echo "============================================"
  echo "  cloud-mail-global — Teardown (--destroy)"
  echo "============================================"
  echo
  preflight
  load_state
  ...
  echo "    Worker:  $WORKER_NAME (in account $ACCOUNT)"
  [ -n "${D1_NAME:-}" ] && echo "    D1:      $D1_NAME ($D1_ID)" || echo "    D1:      cloud-mail-global (will look up)"
  [ -n "${KV_NAME:-}" ] && echo "    KV:      $KV_NAME ($KV_ID)" || echo "    KV:      cloud-mail-global-kv (will look up)"
  [ -n "${R2_BUCKET:-}" ] && echo "    R2:      $R2_BUCKET" || echo "    R2:      cloud-mail-global-r2 (will look up)"
  ...
  # 1. Delete Worker
  step "1/5" "Deleting Worker '$WORKER_NAME'..."
  if npx wrangler delete --name "$WORKER_NAME" --force >/dev/null 2>&1; then
    ok "Worker deleted"
  else
  ...
  local r2="${R2_BUCKET:-cloud-mail-global-r2}"
  ...
  local kv_id="${KV_ID:-}"
  if [ -z "$kv_id" ]; then
    kv_id=$(npx wrangler kv namespace list 2>/dev/null | jq -r --arg n "${KV_NAME:-cloud-mail-global-kv}" '.[] | select(.title==$n) | .id' | head -1)
  fi
  if [ -n "$kv_id" ] && [ "$kv_id" != "null" ]; then
  ...
  local d1_name="${D1_NAME:-cloud-mail-global}"
...
# --- Main ---
echo "============================================"
echo "  cloud-mail-global — One-Click Deploy"
echo "============================================"
echo
...
if [ "$BOOTSTRAP_DOMAIN" = "true" ]; then
  ...
  CF_API_TOKEN="$CF_API_TOKEN" bash "$SCRIPT_DIR/cf-bootstrap-domain.sh" \
    "$PRIMARY_DOMAIN" "$WORKER_NAME" "$ADMIN" || warn "cf-bootstrap-domain returned non-zero — review output above"
fi
