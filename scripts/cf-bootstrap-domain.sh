...
#   3. Set catch-all rule → cloud-mail-global Worker
...
RULE_BODY=$(jq -nc \
  --arg worker "$WORKER" \
  --arg name "cloud-mail-global catch-all (managed)" \
...
============================================
  ...
  For send_email outbound (CF Email Sending):
    ...
    Then in cloud-mail-global, register the admin user → Settings → Email Sending.
